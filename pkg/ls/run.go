...
// Run executes ls with args as if running in the execution context ctx.
func Run(ctx pkg.ExecContext, args []string) int {
	// Extract the needed values from ctx to call Command
...
func (c cmd) listName(stringer Stringer, d string, prefix bool) error {
	...
	err := filepath.Walk(d, func(path string, osfi os.FileInfo, err error) error {
		f := file{
			path: path,
			osfi: osfi,
		}
		...
		if osfi != nil && !errors.Is(err, os.ErrNotExist) {
			f.lsfi = FromOSFileInfo(path, osfi)
			if err != nil && path == d {
				f.err = err
			}
		} else {
...
func indicator(fi FileInfo) string {
